...
client.on('message', message => {
    if (message.content === 'p!confirm') {
        ...
        message.channel.send({embed: {
            color: 3447003,
            description: "p!help - tells you all the commands, p!version - send the current version, p!ping - Responds with Pong!, p!pikapic - Sends a random pikachu picture, p!pokegif - sends a random pokemon gif, p!github - sends the github repository link, p!coinflip - flips a coin, p!developers - mentions the main developers, p!patchnotes - displays the patch notes"
          }});
    }
});
client.login('TOKEN')
